...
class TestKbIsolation(unittest.TestCase):
    ...
    def test_ask_all_scopes_to_own_kbs_with_rerank(self):
        """隔离红线回归：开启 rerank 后，普通用户选「全部」仍只召回自己库，绝不越库。

        rerank 只对「已按 kb 隔离召回的候选」重排，不参与范围决策；本用例确保接入 rerank
        没有意外扩大检索范围（评测证明 rerank 不碰 where 过滤，但红线功能必须有回归保护）。
        """
        ...
        rerank_service.RERANK_PROVIDER = "fake"  # 确定性、零网络
        try:
            alice_h = self._hdr("alice", "alice123")
            bob_h = self._hdr("bob", "bob123")
